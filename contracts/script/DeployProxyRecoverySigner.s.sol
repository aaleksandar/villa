...
contract DeployProxyRecoverySigner is Script {
    /// @notice Deploy BiometricRecoverySignerV2 with proxy
    ...
    function run(address livenessVerifier, address owner)
        external
    ...
    /// @dev Deploys a mock verifier and uses msg.sender as owner
    /// @return proxy The proxy address
    /// @return implementation The implementation address
    function runWithMock() external returns (address proxy, address implementation) {
        vm.startBroadcast();
        ...
        return this.run(verifierAddress, msg.sender);
    }
}
