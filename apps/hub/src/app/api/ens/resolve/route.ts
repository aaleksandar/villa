...
const NICKNAME_RESOLVER_V3_ABI = [
  {
    ...
    type: "function",
    inputs: [{ name: "nicknameHash", type: "bytes32" }],
...
async function resolveOnChain(nickname: string): Promise<`0x${string}` | null> {
  ...
  try {
    ...
    const nicknameHash = keccak256(toBytes(nickname.toLowerCase()));
    const address = await client.readContract({
      address: contractAddress,
      abi: NICKNAME_RESOLVER_V3_ABI,
      functionName: "nicknameToAddress",
      args: [nicknameHash],
    });

    const zeroAddress = "0x0000000000000000000000000000000000000000";
    return address !== zeroAddress ? (address as `0x${string}`) : null;
  } catch {
    return null;
  }
}
...
function parseCallData(callData: `0x${string}`): {
  ...
  if (selector === "0xf1cb7e06") {
    ...
    return {
      selector,
      node: decoded[0] as `0x${string}`,
      coinType: decoded[1] as bigint,
    };
  }
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      if (typeof dnsName === "string" && dnsName.startsWith("0x")) {
        const bytes = new Uint8Array(
          (dnsName.slice(2).match(/.{2}/g) || []).map((byte) =>
            parseInt(byte, 16),
          ),
        );
    ...
    if (
      selector === "0x3b3b57de" ||
      (selector === "0xf1cb7e06" && coinType === 60n)
    ) {
      const zeroAddress = "0x0000000000000000000000000000000000000000";

      const onChainAddress = await resolveOnChain(nickname);
      if (onChainAddress) {
