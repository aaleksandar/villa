 ...
 * - New user onboarding (WelcomeStep with "Get Started")
 * - Returning user sign-in
 * - Auth iframe/popup flows (VillaAuthScreen at /auth)
 ...
 *
 * ARCHITECTURE:
 * - /onboarding → WelcomeStep (Get Started button → opens iframe)
 * - /auth → VillaAuthScreen (Sign In + Create Villa ID buttons, rendered in iframe)
 */
...
test.describe('Authentication Flows', () => {
  ...
  test.describe('New User Onboarding Flow', () => {
    ...
    test('shows welcome screen with get started button', async ({ page }) => {
      ...
      // Check for main elements - WelcomeStep
      ...
      // Check for Get Started button (opens iframe to /auth)
  ...
  test.describe('Auth Iframe/Popup Flow - VillaAuthScreen at /auth', () => {
    test('auth page renders VillaAuthScreen component', async ({ page }) => {
      await page.goto('/auth?appId=test-app')
      await page.waitForLoadState('networkidle')

      // VillaAuthScreen has "Your identity. No passwords." headline
    ...
    test('auth page accepts appId query parameter', async ({ page }) => {
      ...
      // Page should load successfully with VillaAuthScreen headline
      await expect(page.getByRole('heading', { name: /your identity/i })).toBeVisible({ timeout: 10000 })
    })
    ...
    test('auth page works with origin parameter', async ({ page }) => {
      ...
      // Should render successfully with VillaAuthScreen headline
      await expect(page.getByRole('heading', { name: /your identity/i })).toBeVisible({ timeout: 10000 })
    })
    ...
    test('VillaAuthScreen displays supported providers', async ({ page }) => {
      await page.goto('/auth?appId=test')
      await page.waitForLoadState('networkidle')

      // Check for provider logos (VillaAuthScreen shows device biometric providers)
    ...
    test('VillaAuthScreen shows device biometric messaging', async ({ page }) => {
      ...
      // VillaAuthScreen uses "device biometric" terminology
  ...
  test.describe('Logout Flow', () => {
    ...
    test('after logout, can see welcome screen again', async ({ page }) => {
      ...
      // Should see welcome screen - WelcomeStep with "Welcome to Villa"
      await expect(page.getByRole('heading', { name: /welcome to villa/i })).toBeVisible({ timeout: 10000 })
  ...
  test.describe('Error Handling', () => {
    test('shows error step UI correctly', async ({ page }) => {
      ...
      // Click Get Started to trigger auth flow
      const getStartedButton = page.getByRole('button', { name: /get started/i })
      await getStartedButton.waitFor({ state: 'visible', timeout: 10000 })
      
      // Note: We can't fully test the iframe auth flow in Playwright
  ...
  test.describe('Mobile Responsiveness', () => {
    ...
    test('welcome screen renders correctly on mobile', async ({ page }) => {
      ...
      // Check all elements are visible and in viewport - WelcomeStep
      await expect(page.getByRole('heading', { name: /welcome to villa/i })).toBeVisible({ timeout: 10000 })
      await expect(page.getByRole('heading', { name: /welcome to villa/i })).toBeInViewport()
