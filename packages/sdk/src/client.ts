...
import {
  resolveEns as resolveEnsName,
  reverseEns as reverseEnsAddress,
} from "./ens";
...
export class Villa {
  ...
  constructor(config: VillaConfig) {
    ...
    // Use key.villa.cash for SDK iframe auth (dedicated iframe endpoint)
    this.authUrl =
      this.config.network === "base-sepolia"
        ? "https://beta-key.villa.cash/auth"
